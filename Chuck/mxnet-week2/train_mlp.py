...
if __name__=="__main__":
   ...
   # model = get_mlp_sym()
    ...
    plot = mx.viz.plot_network(model_conv, title="Convolution", save_format="pdf", hide_weights=True)
    ...
    mod_conv = mx.mod.Module(symbol=model_conv, context=mx.cpu())
    mod_conv.fit(train_iter,  # train data
            eval_data=val_iter,  # validation data
            optimizer='sgd',  # use SGD to train
            optimizer_params={'learning_rate': 0.1},  # use fixed learning rate
            eval_metric='acc',  # report accuracy during training
